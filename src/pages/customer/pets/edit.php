...
$pet_id = $_GET['id'];
$owner_id = $_SESSION['user_id']; // ID pemilik yang sedang login

$pdo = getDBConnection();

// 3. KEAMANAN: Cek apakah hewan ini milik user yang login
$sql_check = "SELECT * FROM pet WHERE pet_id = ? AND owner_id = ?";
$stmt_check = $pdo->prepare($sql_check);
$stmt_check->execute([$pet_id, $owner_id]);
$pet = $stmt_check->fetch(PDO::FETCH_ASSOC);

// 4. Jika data tidak ditemukan (artinya bukan miliknya atau datanya tidak ada)
if (!$pet) {
    echo "<div class='alert alert-danger'>Data hewan tidak ditemukan atau Anda tidak memiliki hak akses.</div>";
    include_once __DIR__ . '/../layouts/footer.php';
    exit;
}
?>
...
<form action="/actions/customer/pets/update.php" method="POST">
    ...
    <div class="mb-3">
        ...
        <select class="form-select" id="status" name="status" required>
            <option value="Aktif" <?php if($pet['status'] == 'Aktif') echo 'selected'; ?>>Aktif</option>
            <option value="Meninggal" <?php if($pet['status'] == 'Meninggal') echo 'selected'; ?>>Meninggal</option>
        </select>
    </div>
